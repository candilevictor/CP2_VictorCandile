...
/**
 * Classe principal do Sistema de Streaming de Música.
 * Ponto de entrada da aplicação e controlador dos menus.
 */
public class StreamingMusica {

    // Lista polimórfica: armazena UsuarioFree e UsuarioPremium sob tipo base Usuario
    private static ArrayList<Usuario> usuarios = new ArrayList<>();
    private static Usuario            usuarioLogado = null;

    // Catálogo global de músicas
    ...
    private static void criarUsuario() {
        ...
        // Verifica email duplicado — percorre ArrayList polimórfico
        ...
        try {
            if (tipo == 2) {
                ...
                // Upcasting implícito: UsuarioPremium → Usuario
    ...
    private static void fazerLogin() {
        ...
        // Polimorfismo: referência base aponta para objeto específico
    ...
    // ── Listar usuários com instanceof ────────────────────────
    ...
    private static void listarUsuarios() {
        ...
        for (int i = 0; i < usuarios.size(); i++) {
            ...
            // instanceof + downcasting para exibir info específica de cada tipo
    ...
    private static void exibirMenu() {
        // Polimorfismo no menu: exibe opções conforme tipo real do usuário
    ...
    private static void processarOpcao8() {
        if (usuarioLogado instanceof UsuarioPremium) {
            menuDownload((UsuarioPremium) usuarioLogado); // downcasting
        } else {
    ...
    private static void reproduzirMusica() {
        if (catalogo.isEmpty()) { System.out.println("Nenhuma música no catálogo."); return; }
        listarMusicas();
        System.out.print("Escolha a música: ");
        int idx = lerOpcao() - 1;
        if (idx < 0 || idx >= catalogo.size()) { System.out.println("Índice inválido."); return; }

        // Polimorfismo: chama a versão correta de reproduzirMusica() para o tipo real
    ...
    private static void criarPlaylist() {
        ...
        String nome = scanner.nextLine();
        try {
            usuarioLogado.criarPlaylist(nome); // polimorfismo
        } catch (IllegalArgumentException e) {
            System.out.println("Erro: " + e.getMessage());
        }
    }
    ...
    private static void reproduzirPlaylist() {
        ...
        Playlist pl = usuarioLogado.getPlaylist(lerOpcao() - 1);
        if (pl == null) return;
        pl.reproduzir(); // polimorfismo via interface Reproduzivel
    }
    ...
    private static void menuPlaylistsAutomaticas() {
        ...
        try {
            // GeradorRecomendacoes cria a Playlist — desacoplado do menu
        ...
        switch (lerOpcao()) {
            case 1:
                ...
                if (idx >= 0 && idx < catalogo.size()) {
                    premium.baixar(catalogo.get(idx)); // método da interface Baixavel
                } else {
                    System.out.println("Índice inválido.");
                }
                break;
            ...
            case 2:
                ...
                if (idxR >= 0 && idxR < baixadas.size()) {
                    premium.removerDownload(baixadas.get(idxR)); // método da interface Baixavel
                } else {
                    System.out.println("Índice inválido.");
                }
                break;
    ...
    private static void exibirEstatisticasPessoais() {
        ...
        // instanceof + downcasting para dados específicos do tipo Free
    ...
    private static void exibirEstatisticasSistema() {
        ...
        // Percorre ArrayList polimórfico — usa instanceof para dados específicos
        for (Usuario u : usuarios) {
            if (u instanceof UsuarioPremium) {
                UsuarioPremium p = (UsuarioPremium) u;
                totalPremium++;
                repPremium += u.getTotalReproducoes();
                downloads  += p.getTamanhoBaixados(); // método de Baixavel
        ...
        System.out.printf("Usuários: %d  (Free: %d | Premium: %d)%n",
                totalU, totalFree, totalPremium);
